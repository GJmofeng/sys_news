...
<body>
	...
	<div class="layui-col-md12">
		<div class="layui-card">
			<div class="layui-card-header">新闻类型列表</div>
			<div class="layui-card-body">
				<form action="CategoryServlet?action=CategoryList" method="post">
					...
					<tbody>
						<c:forEach items="${list}" var="u">
							<tr>
								...
								<a href="CategoryServlet?action=toUpdateCategory&id=${u.id}" 
               ...
               <div style="text-align: center">
					<div class="layui-btn-group">
						<a class="layui-btn layui-btn-primary"
							href="CategoryServlet?action=CategoryList&p=1">首页</a>
						<%--        判断是否有上一页--%>
						<c:if test="${cp>1}">
							<a class="layui-btn layui-btn-primary"
								href="CategoryServlet?action=CategoryList&p=${cp-1}">上一页</a>
						</c:if>
						<%--        循环显示页码--%>
						<c:forEach begin="${cp-2>1 ? (cp-2) :1}"
							...
							<c:if test="${cp==e}">
								<a class="layui-btn layui-btn-normal"
									href="CategoryServlet?action=CategoryList&p=${e}">${e}</a>
							</c:if>
							<c:if test="${cp!=e}">
								<a class="layui-btn layui-btn-primary"
									href="CategoryServlet?action=CategoryList&p=${e}">${e}</a>
						...
						<c:if test="${cp<tp}">
							<a class="layui-btn  layui-btn-primary"
								href="CategoryServlet?action=CategoryList&p=${cp+1}">下一页</a>
						</c:if>
						<a class="layui-btn  layui-btn-primary"
							href="CategoryServlet?action=CategoryList&p=${tp}">尾页</a> 
					</div>
				</div>
	...
	/*  setTimeout(function(){ $("#msg3").html("")},3000); */
	 ...
	  $('a.layui-btn-danger').click(function(e){
   		            var id = $(e.currentTarget).parent().find('.weui-input').attr('id');
			   		if(confirm("确定要删除吗？")){
			   		  $.post("CategoryServlet?action=deleteCategory&id="+id, function(data) {
